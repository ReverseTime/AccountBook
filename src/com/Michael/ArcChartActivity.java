    ...
    public void income_clickButton(View view) {
        switch (view.getId()) {
            ...
            case R.id.button_arc_search:
                ...
                List<Map<String, Object>> salaryList = dbHelper.selectList(
                        "select money from tb_myAccount where date > dateBeginString and date < dateEndString and bigKind='工资'", null);

                for (int i = 0; i < salaryList.size(); i++) {
                    int s = Integer.parseInt(salaryList.get(i).get("money").toString());
                    salarySum+=s;
                }
                List<Map<String, Object>> perkList = dbHelper.selectList(
                        "select money from tb_myAccount where bigKind='外快'", null);
                for (int i = 0; i < perkList.size(); i++) {
                    int s = Integer.parseInt(perkList.get(i).get("money").toString());
                    perkSum+=s;
                }
                Log.i("---------------------->>>","salarySum"+salarySum+".........perkSum"+perkSum);
